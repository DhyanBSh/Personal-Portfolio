...
export const Testimonials = () => {
  ...
  const isTrackVisibleRef = useRef(false);
  ...
  useEffect(() => {
    ...
    const visibilityObserver = new IntersectionObserver(
      ([entry]) => {
        isTrackVisibleRef.current = entry.isIntersecting;
      },
      { threshold: 0.05 },
    );
    ...
    const getCycleWidth = () => {
      const secondSetStart = track.children[testimonials.length] as HTMLElement | undefined;
      return secondSetStart?.offsetLeft ?? track.scrollWidth / 2;
    };

    track.scrollLeft = getCycleWidth();

    const scrollTrack = (time: number) => {
      ...
      if (
        ...
        isTrackVisibleRef.current &&
        !isAutoScrollPausedRef.current
      ) {
        track.scrollLeft += elapsed * 0.014;
      ...
      animationFrameRef.current = requestAnimationFrame(scrollTrack);
    };
  ...
  const handlePointerMove = (event: PointerEvent<HTMLDivElement>) => {
    ...
    trackRef.current.scrollLeft = dragStartScrollRef.current - (event.clientX - dragStartXRef.current);
  };
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="flex cursor-grab snap-x snap-mandatory gap-6 overflow-x-auto pb-2 [scrollbar-width:none] [touch-action:pan-y] [&::-webkit-scrollbar]:hidden active:cursor-grabbing"
      >
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`flex h-11 w-11 items-center justify-center rounded-none border transition-colors ${
              isDark ? 'border-white/25 hover:bg-white hover:text-black' : 'border-black/20 hover:bg-black hover:text-white'
            }`}
          >
